"""
Media Import Cog for Server Bot

Integrates media import functionality into the existing MeatLizard server bot.
"""
import asyncio
import logging
import re
...
# Import from the web app services
import sys
from pathlib import Path
sys.path.append(str(Path(__file__).parent.parent.parent / "web"))

from app.services.media_import_service import MediaImportService, ImportConfig, MediaExtractionError
from app.services.import_job_queue import ImportJobQueue
from app.models import User, ImportJob

logger = logging.getLogger(__name__)

class MediaImportCog(commands.Cog):
    """Cog for handling media imports via Discord"""
    
    def __init__(self, bot: commands.Bot, db_session_factory, import_channel_id: int):
        self.bot = bot
    ...
    @app_commands.command(name="import", description="Import media from URL")
    @app_commands.describe(
        url="URL of the media to import",
        quality="Quality preset (low, standard, high, audio)"
    )
    async def import_command(self, interaction: discord.Interaction, url: str, quality: str = "standard"):
        """Slash command for importing media"""
        if not await self._is_admin_user(interaction.user):
            await interaction.response.send_message("❌ Only administrators can use this command.", ephemeral=True)
            return
        
        if not await self._is_supported_url(url):
            await interaction.response.send_message("❌ Unsupported platform or invalid URL.", ephemeral=True)
            return
        
        # Quality presets
        quality_configs = {
            "low": {"max_height": 480, "max_fps": 30},
            "standard": {"max_height": 720, "max_fps": 30},
            "high": {"max_height": 1080, "max_fps": 30},
            "audio": {"audio_only": True, "audio_format": "mp3"}
        }
        
        config = quality_configs.get(quality, quality_configs["standard"])
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=f"Importing from: {url}",
                color=0x007bff
            )
            message = await interaction.followup.send(embed=embed)
            
            await self._initiate_import(message, url, config, interaction.user)
            
        except Exception as e:
            logger.error(f"Failed to start import via slash command: {str(e)}")
            await interaction.followup.send(f"❌ Failed to start import: {str(e)}", ephemeral=True)
    
    @app_commands.command(name="import-status", description="Check status of import jobs")
    async def import_status_command(self, interaction: discord.Interaction):
        """Check status of recent import jobs"""
        if not await self._is_admin_user(interaction.user):
            await interaction.response.send_message("❌ Only administrators can use this command.", ephemeral=True)
            return
        ...
        try:
            async with self.db_session_factory() as db:
                ...
                # Get recent import jobs (last 10)
                jobs = await service.get_import_jobs(limit=10)
                
                if not jobs:
                    await interaction.response.send_message("No recent import jobs found.", ephemeral=True)
                    return
                ...
                for job in jobs[:5]:  # Show top 5
                    ...
                    embed.add_field(
                        name=f"{status_emoji} {job.original_title or 'Untitled'}",
                        value=f"Platform: {job.platform}\nStatus: {job.status}\nProgress: {job.progress_percent}%",
                        inline=True
                    )
            ...
            await interaction.response.send_message("❌ Failed to get import status.", ephemeral=True)
    ...
    async def _initiate_import(self, message: discord.Message, url: str, 
        ...
        try:
            async with self.db_session_factory() as db:
                # Get or create user record (simplified - in real implementation, map Discord users to system users)
    ...
    async def _update_progress_status(self, message: discord.Message, job):
    ...
    async def _update_final_status(self, message: discord.Message, job):
    ...
    async def _is_admin_user(self, user: discord.User) -> bool:
        ...
        # For interactions, check guild permissions
        guild = self.bot.get_guild(user.guild.id) if hasattr(user, 'guild') else None
        if guild:
...
async def setup(bot: commands.Bot):
    ...
    # Get database session factory (this would be passed from the main bot)
    db_session_factory = getattr(bot, 'db_session_factory', None)
    if not db_session_factory:
        logger.warning("No database session factory found, media import cog will not function properly")
        return
    
    # Get import channel ID from environment
    import_channel_id = int(os.getenv('DISCORD_IMPORT_CHANNEL_ID', '0'))
    if not import_channel_id:
        logger.warning("DISCORD_IMPORT_CHANNEL_ID not set, media import cog will not function properly")
        return
    
    await bot.add_cog(MediaImportCog(bot, db_session_factory, import_channel_id))